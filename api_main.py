...
accessible_tables = ("generators",
                    "substations",
                    "transmission_lines",
                    "storage",    #updated from "storage_batteries" To "storage"
                    "interties",
                    "junctions",
                    ...
                    #updated with new tables, and tables that had name changes.
...
@app.route('/<string:table>', methods=['GET'])          #returns any full table request
@Limiter.limit('100 per second')
def return_table(table):
    ...
    ###Correct spot to add new lines for new tables??
...
def return_annual_demand_and_efficiencies(region):
    ...
    result = send_query(query)
    column_names = get_columns("annual_demand_and_effeciencies")    #mistake??? - spelt wrong in db too
...
def return_based_on_prov(table, province):
    ...
    elif table in ("generators","transmission_lines","storage","hydro_existing"):  #updated storage_batteries to just storage
        query = f"SELECT * FROM  {table} WHERE province = '{province}'"
    else:
...
def return_generator_type(province, gen_type):
    ...
    #####correct spot to add new functions for each new table???
